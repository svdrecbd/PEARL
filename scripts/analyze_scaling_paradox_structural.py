...
def validate_matrix(
    ...
    *,
    ...
    expected_jobs = {
        ...
        if row["campaign"] in {"shared_base", "original" if config["campaign_id"] == "pearl-scaling-paradox-v1" else "replication"}
    }
...
def main() -> None:
    ...
    if structural_manifest.get("contract") != "pearl.scaling-paradox-structural-manifest/1":
    ...
    capacity_contrasts = [
        {
            "training_seed": seed,
            "clean_4b_minus_9b": true_shuffled[(tags["qwen3p5-4b"], seed)]
            - true_shuffled[(tags["qwen3p5-9b"], seed)],
            "extension_4b_minus_27b": true_shuffled[(tags["qwen3p5-4b"], seed)]
            - true_shuffled[(tags["qwen3p6-27b"], seed)],
        }
        for seed in seeds
    ]
    ...
    payload = {
        ...
        "capacity_inference": {
            "clean_4b_minus_9b": seed_mean_interval(
                [row["clean_4b_minus_9b"] for row in capacity_contrasts]
            ),
            "extension_4b_minus_27b": seed_mean_interval(
                [row["extension_4b_minus_27b"] for row in capacity_contrasts]
            ),
        },
