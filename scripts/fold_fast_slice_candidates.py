...
def fold_sequence(seq: str, name: str, out_dir: Path) -> Path:
    ...
    if out_path.exists():
        print(f"[{name}] Found cached PDB.")
        return out_path
    
    url = "https://api.esmatlas.com/foldSequence/v1/pdb/"
    print(f"[{name}] Folding sequence via ESMAtlas API...")
    try:
...
def main():
    ...
    # We want to fold Step 0, Step 2, Step 4, Step 9, Step 11
    ...
    print("\n--- Downstream Fold Verification Results ---")
    ...
    print(f"\nSaved PDB files and metrics to {out_dir}")
