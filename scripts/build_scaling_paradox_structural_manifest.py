#!/usr/bin/env python3
"""Build the exact 111-cell structural-generation manifest from audited core receipts."""
...
def main() -> None:
    ...
    structural_configs = {
        "original": ROOT / "configs/experiments/scaling_paradox_structural_v1.json",
        "replication": ROOT / "configs/experiments/scaling_paradox_structural_v1_replication.json",
    }
    ...
    schedules = {
        "original": [500, 1000, 1500, 2000, 2250],
        "replication": [2250],
    }
    for cohort in ("original", "replication"):
        ...
        for entry in plan["runs"]:
            ...
            for step in schedules[cohort]:
                ...
                jobs.append(job)
    if len(jobs) != 111 or len({job["job_key"] for job in jobs}) != 111:
        raise RuntimeError("structural manifest must contain exactly 111 unique cells")
    waves = [
        {
    ...
    payload = {
        "contract": "pearl.scaling-paradox-structural-manifest/1",
        ...
        "scope": "three shared base cells; original core at 500/1000/1500/2000/2250; replication core terminal only",
        "excluded_stages": ["data_exposure", "adapter_rescue"],
