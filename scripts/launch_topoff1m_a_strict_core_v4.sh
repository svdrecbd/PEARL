...
POSTPROCESS_DIR="$ROOT/reports/raft/topoff1m-a-strict-core-v4-postprocess-20260329"
...
case "$STAGE" in
  stage-a)
    RUN_NAME="pearl-micro-sft-topoff1m-a-strict-core-v4-stagea-lr1e6-ep2"
    DATASET_PATH="$POSTPROCESS_DIR/strict_core_v4_stage_a.jsonl"
    ...
    ;;
  stage-b-lite)
    RUN_NAME="pearl-micro-sft-topoff1m-a-strict-core-v4-stageb-lite-lr5e7-ep1"
    DATASET_PATH="$POSTPROCESS_DIR/strict_core_v4_stage_b_lite.jsonl"
    STAGE_A_SUMMARY_PATH="$ROOT/reports/warmstart/pearl-micro-sft-topoff1m-a-strict-core-v4-stagea-lr1e6-ep2/summary.json"
    if [[ -n "${STRICT_CORE_V4_STAGE_A_CHECKPOINT:-}" ]]; then
      INIT_STATE_PATH="$STRICT_CORE_V4_STAGE_A_CHECKPOINT"
...
PY
      ...
      echo "Stage A summary is missing; set STRICT_CORE_V4_STAGE_A_CHECKPOINT or run stage-a first" >&2
      exit 1
    fi
    LEARNING_RATE="5e-7"
    EPOCHS="1"
