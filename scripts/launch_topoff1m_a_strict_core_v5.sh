...
case "$STAGE" in
  stage-a)
    RUN_NAME="pearl-micro-sft-topoff1m-a-strict-core-v5-stagea-lr1e6-ep3"
    DATASET_PATH="$POSTPROCESS_DIR/strict_core_v5_stage_a.jsonl"
    ...
    ;;
  stage-b-lite)
    RUN_NAME="pearl-micro-sft-topoff1m-a-strict-core-v5-stageb-lite-lr5e7-ep1"
    DATASET_PATH="$POSTPROCESS_DIR/strict_core_v5_stage_b_lite.jsonl"
    STAGE_A_SUMMARY_PATH="$ROOT/reports/warmstart/pearl-micro-sft-topoff1m-a-strict-core-v5-stagea-lr1e6-ep3/summary.json"
    if [[ -n "${STRICT_CORE_V5_STAGE_A_CHECKPOINT:-}" ]]; then
      INIT_STATE_PATH="$STRICT_CORE_V5_STAGE_A_CHECKPOINT"
...
PY
      ...
      echo "Stage A summary is missing; set STRICT_CORE_V5_STAGE_A_CHECKPOINT or run stage-a first" >&2
      exit 1
    fi
    LEARNING_RATE="5e-7"
    EPOCHS="1"
