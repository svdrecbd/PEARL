...
python "$ROOT/scripts/build_strict_first_union_curricula.py" \
  ...
  --stage-a-output-path "$POSTPROCESS_DIR/strict_core_v2_stage_a.jsonl" \
  --stage-a-summary-path "$POSTPROCESS_DIR/strict_core_v2_stage_a_summary.json" \
  --stage-b-output-path "$POSTPROCESS_DIR/strict_core_v2_stage_b_lite.jsonl" \
  --stage-b-summary-path "$POSTPROCESS_DIR/strict_core_v2_stage_b_lite_summary.json" \
  --old-repeat 2 \
  --new-repeat 3 \
  --pure-repeat 2 \
  --anchor-count 6
