...
ssh -i "$SSH_KEY_PATH" "$REMOTE_USER@$HOST" \
  "mkdir -p '$REMOTE_ROOT/scripts' '$REMOTE_ROOT/src/pearl' '$REMOTE_ROOT/data/petase_family_expanded' '$REMOTE_ROOT/reports/logs' '$REMOTE_ROOT/reports/robustness' '$REMOTE_ROOT/reports/ablations' '$REMOTE_ROOT/notes'"
...
rsync -av -e "ssh -i $SSH_KEY_PATH" \
  ...
  "$REPO_ROOT/scripts/run_robustness_suite.py" \
  "$REPO_ROOT/scripts/run_robustness_two_phase.py" \
  "$REPO_ROOT/scripts/launch_detached_job.py" \
  "$REPO_ROOT/scripts/stop_detached_job.py" \
  "$REPO_ROOT/scripts/run_nebius_h100_robustness.sh" \
  "$REPO_ROOT/scripts/launch_topoff1m_a_robustness_h100.sh" \
...
rsync -av -e "ssh -i $SSH_KEY_PATH" \
  ...
  "$REPO_ROOT/data/petase_family_expanded/val_prompts_relevance_ge10.jsonl" \
